...
export class ProductsComponent implements OnInit {
  ...
  async fetchProducts() {
    try {
      ...
      console.warn('Error fetching ML products via API, using local mock data fallback:', e);
      // Fallback local seguro para que a tela não quebre se a API de SSR não estiver rodando no mesmo host
      this.products.set([
        {
          id: 'ML-001',
      ...
      this.error.set(false); // Remove estado de erro para renderizar os mocks locais
    } finally {
